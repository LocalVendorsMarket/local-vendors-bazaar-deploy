  ...
  return (
    <div style={{ fontFamily: 'sans-serif', backgroundColor: '#e6f0ff', minHeight: '100vh' }}>
      ...
      <footer style={{ backgroundColor: '#003366', color: 'white', padding: '2rem', marginTop: '2rem', textAlign: 'center' }}>
        <div>&copy; {new Date().getFullYear()} Local Vendors Bazaar. All rights reserved.</div>
      </footer>
        ...
        <div style={modalStyle} onClick={() => setIsUpdateLocationOpen(false)}>
          <div style={modalContentStyle} onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleUpdateLocationSubmit}>
              <input type="text" value={newZip} onChange={(e) => setNewZip(e.target.value)} placeholder="Enter new zip code" style={{ padding: '10px', borderRadius: '6px', width: '200px', fontSize: '16px' }} />
              <div>
                ...
                <button onClick={() => setIsUpdateLocationOpen(false)} style={{ padding: '0.5rem 1rem', backgroundColor: '#ccc', borderRadius: '6px' }}>Cancel</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div style={modalStyle} onClick={() => setIsSignInModalOpen(false)}>
          <div style={modalContentStyle} onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSignInSubmit}>
              {isNewCustomer && (
                <>
                  <input type="text" value={signInName} onChange={(e) => setSignInName(e.target.value)} placeholder="Name or Company" required style={{ padding: '10px', borderRadius: '6px', width: '100%' }} />
                  <input type="tel" value={signInPhone} onChange={(e) => setSignInPhone(e.target.value)} placeholder="Phone Number" required style={{ padding: '10px', borderRadius: '6px', width: '100%' }} />
                </>
              )}
              <input type="email" value={signInEmail} onChange={(e) => setSignInEmail(e.target.value)} placeholder="Enter your email" required style={{ padding: '10px', borderRadius: '6px', width: '100%' }} />
              <div style={{ marginTop: '1rem', display: 'flex', gap: '10px' }}>
                <button type="submit" style={{ padding: '0.5rem 1rem', backgroundColor: '#003366', color: 'white', borderRadius: '6px' }}>{isNewCustomer ? 'Sign Up' : 'Sign In'}</button>
                <button type="button" onClick={() => setIsNewCustomer(!isNewCustomer)} style={{ padding: '0.5rem 1rem', backgroundColor: '#ccc', borderRadius: '6px' }}>{isNewCustomer ? 'Back to Sign In' : 'New Customer?'}</button>
                <button type="button" onClick={() => setIsSignInModalOpen(false)} style={{ padding: '0.5rem 1rem', backgroundColor: '#aaa', borderRadius: '6px' }}>Cancel</button>
              </div>
            </form>
          </div>
        </div>
